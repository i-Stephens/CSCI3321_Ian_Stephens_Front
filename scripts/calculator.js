...
function solve() {
    ...
    while(postfixStack.length > 0) {                    // clear postfix stack
        postfixStack.pop();
    }
    while(mathStack.length > 0) {                       // clear math stack
        mathStack.pop();
    }
    displayLength = document.getElementById('display').innerHTML.length;
    manPost(document.getElementById('display').innerHTML);
    while(postfixStack.length > displayLength) {        // fix unwanted duplicate values in postfix stack
        postfixStack.pop();
    }
...
function calculateFromPostfix() {
    for(i=0;i<postfixStack.length;i++) {
        ...
        if(getPrecedence(n.item) == -1) {       // if item is not an operator
            stack.push(n);
        } else {
...
function manPost(str) {
    ...
    } else if(getPrecedence(str.substr(0,1)) == -1) {   // if next char is a number
        let i = 1;
    ...
    } else if(stack.length == 0) {                      // next char is not a number and stack is empty
        stack.push(str.substr(0,1));
        return manPost(str.substring(1));
    } else {                                            // next char is operator so check for precedence
